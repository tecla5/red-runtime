...
describe('red/nodes/registry/localfilesystem', function () {
    ...
    describe('#getModuleFiles', function () {
            ...
            it('throws an error if a node isn'
                t found ',function(done) {
                var _join = path.join; stubs.push(sinon.stub(path, 'join', function () {
                    if (arguments[0] == resourcesDir) {
                        // This stops the module tree scan from going any higher
                        // up the tree than resourcesDir.
                        return arguments[0];
            ...
            }); it.skip('finds locales directory'); it.skip('finds icon path directory');
    });
});
